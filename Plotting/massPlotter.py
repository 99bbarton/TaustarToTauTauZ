...
varToPlotParams = { 
    ...
    #"VIS_M"   : ["CH_visM", "Visible Mass [GeV]", 530, 200, 5500], #TODO need to add a per-channel plotting function, mayber just an if + 3x replaces?
    #"MIN_COL_M"   : ["CH_minCollM", "Min Collinear Mass [GeV]", 530, 200, 5500] #TODO
    #"MAX_COL_M"   : ["CH_maxCollM", "Max Collinear Mass [GeV]", 530, 200, 5500] #TODO
}
...
def parseArgs():
    ...
    argparser.add_argument("vars", nargs='+', choices=["SIG_M", "Z_PT", "Z_ETA", "Z_DAUDR", "Z_M", "VIS_M", "COL_M"], help="What to plot. If one argument is provided, a 1D hist of that variable will be produced. If a second argument is also provided, the first arg will be plotted on the x-axis and the second, the y-axis and sim for 3 args.")
    ...
    #print("args.vars = " + str(args.vars) + " which has length " + str(len(args.vars)))
...
def plot1D(filelist, args):
    ...
    for hNum, hName in enumerate(filelist.keys()):
        ...
        for filename in filelist[hName]:
            ...
            tree.Draw(plotParams[0] + ">>+h_"+hName+"_temp", cutStr)
...
def plot2D_hists(filelist, args):
    ...
    for filename in filelist["ALL"]:
        ...
        tree.Draw(plotParamsD2[0] + ":" + plotParamsD1[0] + ">>+h_temp_2d", cutStr)
