...
def haddFiles(args):
    ...
    argparser.add_argument("-d", "--date", required=True, nargs=1, type=str, help="The date in the form e.g. 1Apr2025 that the jobs were created on")
    argparser.add_argument("-y", "--year", required=True, nargs=1, choices=["2016", "2016post", "2017", "2018", "2022", "2022post", "2023", "2023post"], help="What year the jobs correspond to")
    ...
    argparser.add_argument("-v", "--version", required=True, nargs=1, type=str, help="A unique string to denote the new file area on EOS.")
    ...
    for year in args.years:
        inDir = IN_DIR_BASE + year + "/"
        outDir = OUT_DIR_BASE + year + "/" + args.version + "/"
        ...
        for proc in args.processes:
            ...
            print("\thadd'ing " + year + " " + proc +  " samples...")
            for subproc in sub_processes[proc]:
                command = "hadd root://cmseos.fnal.gov/" + outDir + subproc + "_" + year + ".root `xrdfsls -u " + inDir + " | grep " + subproc + "`"
                stdout, stderr  = subprocess.Popen(command, universal_newlines=True, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
